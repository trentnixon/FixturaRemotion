  ...
  return (
    <PlayerROW
      style={{
        ...
        backgroundColor: i === 0 ? hasOpacity(Color.Secondary.Main) : hasOpacity(Color.Primary.Main),
        opacity: interpolateOpacityByFrame(frame, 30 * (5 - i + 1), 40 * (5 - i + 1), 0, 1),
        transform: `translateX(${SpringToFrom(30 * (5 - i + 1), -1440, 0, 'Wobbly')}px) translateX(${SpringToFrom(FPS_MAIN - 30 + i, 0, 1440, 'Wobbly')}px)`,
      }}
    >
      <SmallBoxLeftSide
        style={{
          borderRadius: TemplateVariation.borderRadius,
          background: i === 0 ? hasOpacity(Color.Secondary.Main) : hasOpacity(Color.Primary.Darken),
      ...
      <PlayerMetaContainer>
        <PlayerName
          style={{
            ...
            fontWeight: 600,
            borderRadius: TemplateVariation.borderRadius,
            color: getContrastColor(i === 0 ? Color.Secondary.Main : Color.Primary.Main),
          }}
        >
      ...
      <PlayerScoreContianer
        style={{
          borderRadius: TemplateVariation.borderRadius,
          background: hasOpacity(i === 0 ? Color.Secondary.Darken : Color.Primary.Darken),
          borderColor: i === 0 ? Color.Secondary.Main : Color.Primary.Main,
        }}
      >
        {TYPE === 'BATTING' ? (
          <BasicBattingScores Font={Font.Copy} player={player} COLOR={getContrastColor(i === 0 ? Color.Secondary.Darken : Color.Primary.Darken)} />
        ) : (
          <BasicBowlingScores Font={Font.Copy} player={player} COLOR={getContrastColor(i === 0 ? Color.Secondary.Darken : Color.Primary.Darken)} />
        )}
      </PlayerScoreContianer>
    </PlayerROW>
  );
